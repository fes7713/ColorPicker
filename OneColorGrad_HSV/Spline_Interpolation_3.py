...
def Color_Palette_2D(nSteps):
    color_palette = []
    steps = int(nSteps ** 0.5) ** 2
    steps_for_interpolation = int((int(nSteps ** 0.5) + 1)**2)

    for i in range(steps):
        hue_value = 360 / steps * i

        code_bottom_left = [hue_value, 255, 0]
        code_bottom_right = [hue_value, 0, 0]
        code_top_left = [hue_value, 0, 255]
        code_top_right = [hue_value, 255, 255]

        colors = Color_Interpolation_HSV_2D(code_top_right, code_top_left, code_bottom_left, code_bottom_right,
                                            steps_for_interpolation)
        del colors[-1]
        colors = sum(colors, [])
        del colors[::steps_for_interpolation]
        color_palette.append(colors)
    return color_palette

def Color_Palette_2D_Fixed_Value(nSteps, value):
    color_palette = []

    for i in range(nSteps + 1):
        hue_value = 360 / (nSteps+1) * i
        colors = []
        for j in range(nSteps + 1):
            sat_value = 255/nSteps * j
            colors.append([hue_value, sat_value, value])
        color_palette.append(colors)
    return color_palette

def Color_Palette_2D_Fixed_Sat(nSteps, sat):
    color_palette = []

    for i in range(nSteps):
        hue_value = 360 / nSteps * i
        colors = []
        for j in range(nSteps):
            value = 255/nSteps * j
            colors.append([hue_value, sat, value])
        color_palette.append(colors)
    return color_palette
